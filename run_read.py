...
parser.add_argument('--corruption-modality', type=str, default='video', choices=['video', 'audio', 'none', 'missing'], help='which modality to be corrupted')
...
parser.add_argument('--output', type=str, default='output/READ_vggsound.json', help='the output json file to save the results')
...
output_json = {
    ...
    'corruption_modality': args.corruption_modality,
    'results': []
}
...
for corruption in corruption_list:
    ...
    for severity in range(args.severity_start, args.severity_end+1):
        severity_entry = {"severity": severity, "seeds": [], }
        epoch_accs = []
                ...
                with torch.no_grad(): 
                    for epoch in range(1):
                        ...
                        for i, (a_input, v_input, labels) in enumerate(data_bar):
                            ...
                            # current_epoch_acc = evaluate_model(read_model, tta_loader, device, args)
                            # with open('test-bs16.csv', 'a', encoding='utf-8') as f:
                            #     f.write(f"{corruption},Seed#{seed},Batch#{i},{current_epoch_acc}\n")
                            
                            all_features.append(outputs[1].cpu())
